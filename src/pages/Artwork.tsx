...
export default function Artwork() {
    ...
    const [notesStore, setNotesStore] = useState<Artwork[]>([]);

    // Fetch artists and donors on mount
    useEffect(() => {
        getAllArtist().then(setArtists);
        getAllDonor().then(setDonors);
        getAllArtwork().then(setNotesStore);
    }, []);

    const notesDataSource: DataSource<Artwork> = {
        ...
        getMany: async ({ paginationModel, filterModel, sortModel }) => {
            ...
            await getAllArtwork()
                .then((data) => {
                    setNotesStore(data.map((art: any) => ({
                        id: art.base.id,
                        base: { id: art.base.id },
                        artist: art.artist,
                        donor: art.donor,
        ...
        updateOne: async (noteId, data) => {
            ...
            setNotesStore(notesStore.map((note) => {
        ...
        deleteOne: async (noteId) => {
            ...
            setNotesStore(notesStore.filter((note) => note.id !== Number(noteId)))
