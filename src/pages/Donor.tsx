...
import type { Donor } from '../service/Utils';
import { TextField } from '@mui/material';



let notesStore: Donor[];

export const notesDataSource: DataSource<Donor> = {
    fields: [
    ...
    getMany: async ({ paginationModel, filterModel, sortModel }) => {
        let processedNotes: Donor[] = notesStore; // Initialize with current notesStore
        await getAllDonor()
            .then((data) => {
                console.log(data)
    ...
    updateOne: async (noteId, data) => {
        ...
        let updatedNote: Donor | null = null;
        ...
        await UpdateDonor(updatedNote, String(noteId))
            .catch((err) => {
                console.error('Error fetching Donor:', err);
            });
    ...
    deleteOne: async (noteId) => {
        ...
        await DeleteDonor(String(noteId))
            .catch((err) => {
                console.error('Error fetching Donor:', err);
            });
    },
    ...
    validate: (formValues) => {
        let issues: { message: string; path: [keyof Donor] }[] = [];
    ...
    return (
        ...
        <div style={{ width: '100%' }}>
            <Crud<Donor>
                ...
                rootPath="/catalog/donor"
                initialPageSize={25}
                // defaultValues={{ first_name: 'New Donor' }}
                pageTitles={{
                    create: 'New Donor',
                    edit: `Donor ${noteId} - Edit`,
                    show: `Donor ${noteId}`,
                }}
            />
        </div>
